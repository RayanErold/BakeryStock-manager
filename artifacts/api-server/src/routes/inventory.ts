  ...
  try {
    ...
    if (currentUser.role === "staff") {
      ...
      if (branchId) {
        ...
        conditions.push(eq(branchesTable.organizationId, currentUser.organizationId ?? ""));
      }
    }
...
router.get("/inventory/barcode/:code", requireAuth, async (req: AuthedRequest, res: Response) => {
  try {
    ...
    const conditions: Parameters<typeof and>[0][] = [eq(inventoryItemsTable.barcode, code)];
...
router.post("/inventory", requireAuth, async (req: AuthedRequest, res: Response) => {
  try {
    ...
    const [item] = await db
      ...
      .values({ name, category, quantity: String(quantity), unit, minThreshold: String(minThreshold), branchId, barcode: barcode?.trim() || null })
      .returning();

    const [branch] = await db.select().from(branchesTable).where(eq(branchesTable.id, item.branchId)).limit(1);
    return res.status(201).json({ ...item, branchName: branch?.name ?? null, isLowStock: parseFloat(item.quantity) <= parseFloat(item.minThreshold) });
  } catch (err: unknown) {
    if (isDuplicateBarcodeError(err)) {
...
router.get("/inventory/:id", requireAuth, async (req: AuthedRequest, res: Response) => {
  try {
    ...
    if (currentUser.role === "staff") {
      ...
      if (item.branchId !== currentUser.branchId) return res.status(403).json({ error: "Forbidden" });
      return res.json({ ...item, isLowStock: parseFloat(item.quantity) <= parseFloat(item.minThreshold) });
    }
...
router.put("/inventory/:id", requireAuth, async (req: AuthedRequest, res: Response) => {
  try {
    ...
    if (currentUser.role === "staff") {
      ...
      if (existing.branchId !== currentUser.branchId) return res.status(403).json({ error: "Forbidden: staff can only edit items in their branch" });
    }
    ...
    const [branch] = await db.select().from(branchesTable).where(eq(branchesTable.id, item.branchId)).limit(1);
    return res.json({ ...item, branchName: branch?.name ?? null, isLowStock: parseFloat(item.quantity) <= parseFloat(item.minThreshold) });
  } catch (err: unknown) {
    if (isDuplicateBarcodeError(err)) {
      return res.status(409).json({ error: "A item with that barcode already exists in this branch" });
    }
...
router.delete("/inventory/:id", requireAuth, async (req: AuthedRequest, res: Response) => {
  try {
    ...
    const currentUser = await getCurrentUser(req.clerkUserId);
    if (!currentUser) return res.status(401).json({ error: "User not found" });
    if (currentUser.role === "staff") return res.status(403).json({ error: "Forbidden: only owners can delete inventory items" });
